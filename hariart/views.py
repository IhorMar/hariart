...
from .serializers import (
    PaintingSerializer,
    PaintingSizeSerializer,
    OrderSerializer,
    PaintingOrderSerializer,
    ContactSerializer,
)
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        try:
            ...
            recipients = list(
                map(lambda contact_email: contact_email["email"], contact_emails)
            )
...
def contact_us(request):
    try:
        ...
        contact_emails = Contact.objects.filter(
            country__in=checkCountryByLanguage(post_data["language"])
        ).values("email")
...
def displayPaintingsForMessage(paintingsDetail):
    return "\n".join(
        list(
            map(
                lambda painting: f'{painting["name"]} x {painting["amount"]}',
                paintingsDetail,
            )
        )
    )
...
def checkCountryByLanguage(language):
    ...
    return list(
        map(
            lambda country: country[0],
            filter(
                lambda country: country[0] in countriesRelateToLang, COUNTRY_CHOICES
            ),
        )
    )
